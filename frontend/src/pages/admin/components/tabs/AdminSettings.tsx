...
export function AdminSettings() {
    ...
    const [settings, setSettings] = useState({
        ...
        emailAlertRecipients: [] as string[],
        // AI Tagging Settings
        aiTagging: {
            enabled: false,
            provider: 'fallback', // 'google' | 'aws' | 'fallback'
            googleApiKey: '',
            awsRegion: 'us-east-1',
            awsCredentials: {
                accessKeyId: '',
        ...
        try {
            ...
            if (data.settings) {
                ...
                const loadedSettings = {
                    ...
                    // AI Tagging Settings
                    aiTagging: ((settingsData as any).aiTagging || {
                        enabled: false,
                        provider: 'fallback',
                        googleApiKey: '',
                        awsRegion: 'us-east-1',
                    ...
                    }) as { enabled: boolean; provider: string; googleApiKey: string; awsRegion: string; awsCredentials: { accessKeyId: string; secretAccessKey: string } },
                };
    ...
    useEffect(() => {
        ...
        hasChangesTimeoutRef.current = setTimeout(() => {
            ...
            const changed = settings.siteName !== originalSettings.siteName ||
                ...
                // AI Tagging Settings (shallow object comparison)
                (settings.aiTagging?.enabled !== originalSettings.aiTagging?.enabled ||
                    settings.aiTagging?.provider !== originalSettings.aiTagging?.provider ||
                    settings.aiTagging?.googleApiKey !== originalSettings.aiTagging?.googleApiKey ||
                    settings.aiTagging?.awsRegion !== originalSettings.aiTagging?.awsRegion ||
                    settings.aiTagging?.awsCredentials?.accessKeyId !== originalSettings.aiTagging?.awsCredentials?.accessKeyId ||
                    settings.aiTagging?.awsCredentials?.secretAccessKey !== originalSettings.aiTagging?.awsCredentials?.secretAccessKey);
                    ...
                    <Card className="admin-settings-card">
                        ...
                        <CardContent>
                            <div className="admin-form">
                                ...
                                <div className="admin-settings-section-divider" style={{ marginTop: '2rem' }}>
                                    <h3 className="admin-settings-section-title">
                                        <Zap size={18} style={{ marginRight: '0.5rem' }} aria-hidden="true" />
                                        AI Auto-Tagging
                                    </h3>
                                </div>

                                <div className={`admin-form-group admin-form-group-important ${JSON.stringify(settings.aiTagging) !== JSON.stringify(originalSettings.aiTagging) ? 'has-changes' : ''}`}>
                                    <Label className="admin-form-label-with-icon" htmlFor="ai-tagging-enabled">
                                        ...
                                        <div className="admin-tooltip-wrapper">
                                            <HelpCircle size={14} className="admin-tooltip-icon" aria-hidden="true" />
                                            <span className="admin-tooltip-text" role="tooltip">
                                                Automatically generate tags for uploaded images using AI
                                            </span>
                                        </div>
                                    </Label>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <input
                                            ...
                                            type="checkbox"
                                            checked={settings.aiTagging?.enabled || false}
                                ...
                                {settings.aiTagging?.enabled && (
                                    <>
                                        <div className={`admin-form-group ${settings.aiTagging?.provider !== originalSettings.aiTagging?.provider ? 'has-changes' : ''}`}>
                                            <Label htmlFor="ai-tagging-provider" className="admin-form-label-with-icon">
                                                <Server size={16} className="admin-form-label-icon" aria-hidden="true" />
                                                AI Provider
                                            </Label>
                                            <select
                                                id="ai-tagging-provider"
                                                value={settings.aiTagging?.provider || 'fallback'}
                                                onChange={(e) => setSettings(prev => ({
                                                    ...prev,
                                                    aiTagging: {
                                                        ...prev.aiTagging,
                                            ...
                                            >
                                                <option value="fallback">None (Manual tags only)</option>
                                                <option value="google">Google Cloud Vision</option>
                                                <option value="aws">AWS Rekognition</option>
                                            </select>
                                            <p className="admin-form-help-text">
                                                Select the AI service to use for tag generation
                                            </p>
                                        </div>

                                        {settings.aiTagging?.provider === 'google' && (
                                            <div className={`admin-form-group admin-form-group-important ${settings.aiTagging?.googleApiKey !== originalSettings.aiTagging?.googleApiKey ? 'has-changes' : ''}`}>
                                                <Label htmlFor="google-api-key" className="admin-form-label-with-icon">
                                                    <Lock size={16} className="admin-form-label-icon" aria-hidden="true" />
                                                    Google Vision API Key
                                                    <span className="admin-setting-importance-badge admin-setting-importance-important">Required</span>
                                                </Label>
                                                <Input
                                                    id="google-api-key"
                                                    type="password"
                                                    value={settings.aiTagging?.googleApiKey || ''}
                                                    onChange={(e) => setSettings(prev => ({
                                                        ...prev,
                                                        aiTagging: {
                                                            ...prev.aiTagging,
                                                ...
                                                <p className="admin-form-help-text">
                                                    ...
                                                    <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener noreferrer" style={{ color: '#3b82f6' }}>
                                        ...
                                        {settings.aiTagging?.provider === 'aws' && (
                                            <>
                                                <div className={`admin-form-group ${settings.aiTagging?.awsRegion !== originalSettings.aiTagging?.awsRegion ? 'has-changes' : ''}`}>
                                                    <Label htmlFor="aws-region" className="admin-form-label-with-icon">
                                                        <Server size={16} className="admin-form-label-icon" aria-hidden="true" />
                                                        AWS Region
                                                    </Label>
                                                    <Input
                                                        id="aws-region"
                                                        type="text"
                                                        value={settings.aiTagging?.awsRegion || 'us-east-1'}
                                                        onChange={(e) => setSettings(prev => ({
                                                            ...prev,
                                                            aiTagging: {
                                                                ...prev.aiTagging,
                                                ...
                                                <div className={`admin-form-group admin-form-group-important ${settings.aiTagging?.awsCredentials?.accessKeyId !== originalSettings.aiTagging?.awsCredentials?.accessKeyId ? 'has-changes' : ''}`}>
                                                    <Label htmlFor="aws-access-key" className="admin-form-label-with-icon">
                                                        <Lock size={16} className="admin-form-label-icon" aria-hidden="true" />
                                                        AWS Access Key ID
                                                        <span className="admin-setting-importance-badge admin-setting-importance-important">Required</span>
                                                    </Label>
                                                    <Input
                                                        id="aws-access-key"
                                                        type="password"
                                                        value={settings.aiTagging?.awsCredentials?.accessKeyId || ''}
                                                        onChange={(e) => setSettings(prev => ({
                                                            ...prev,
                                                            aiTagging: {
                                                                ...prev.aiTagging,
                                                                awsCredentials: {
                                                                    ...prev.aiTagging.awsCredentials,
                                                ...
                                                <div className={`admin-form-group admin-form-group-important ${settings.aiTagging?.awsCredentials?.secretAccessKey !== originalSettings.aiTagging?.awsCredentials?.secretAccessKey ? 'has-changes' : ''}`}>
                                                    <Label htmlFor="aws-secret-key" className="admin-form-label-with-icon">
                                                        <Lock size={16} className="admin-form-label-icon" aria-hidden="true" />
                                                    ...
                                                    <Input
                                                        id="aws-secret-key"
                                                        type="password"
                                                        value={settings.aiTagging?.awsCredentials?.secretAccessKey || ''}
                                                        onChange={(e) => setSettings(prev => ({
                                                            ...prev,
                                                            aiTagging: {
                                                                ...prev.aiTagging,
