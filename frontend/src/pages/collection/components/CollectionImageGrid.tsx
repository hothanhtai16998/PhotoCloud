          ...
          return (
            ...
            >
              <BlurUpImage
                ...
                priority={index < 12}
                onLoadComplete={() => {
                  // Find the loaded image element within this item using data attribute
                  if (!processedImages.current.has(image._id) && currentImageIds.has(image._id)) {
                    const imageItem = document.querySelector(`[data-image-item-id="${image._id}"]`);
                    if (imageItem) {
                      const imgElement = imageItem.querySelector('.blur-up-image.full') as HTMLImageElement;
                      if (imgElement && imgElement.complete && imgElement.naturalWidth > 0) {
                        handleImageLoad(image._id, imgElement);
                      }
                    }
                  }
                }}
