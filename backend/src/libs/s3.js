...
export async function uploadImageWithSizes(buffer, bucket, filename, mimetype = null) {
	try {
		...
		const [
			...
			uploadToR2(thumbnailWebpBuffer, `${bucket}/${filename}-thumbnail.webp`, 'image/webp'),
			uploadToR2(smallWebpBuffer, `${bucket}/${filename}-small.webp`, 'image/webp'),
			uploadToR2(regularWebpBuffer, `${bucket}/${filename}-regular.webp`, 'image/webp'),
		...
		const criticalMs = Date.now() - criticalStart;
		logger.info(`[UPLOAD] Phase 1 complete in ${criticalMs}ms - Critical formats uploaded`);
		
		// Log file sizes for debugging
		logger.info(
			`[UPLOAD] File sizes - Original: ${(originalBuffer.length / 1024 / 1024).toFixed(2)}MB, WebP Full: ${(webpFullBuffer.length / 1024 / 1024).toFixed(2)}MB`
		);
		logger.info(`[UPLOAD] Original URL: ${original}, Extension: ${originalExtension}`);

		// Return critical formats immediately (image can appear now)
		// AVIF formats will be generated in background (Phase 2)
		return {
			publicId: filename,
			// WebP variants (used as universal modern fallback) - CRITICAL
			thumbnailUrl: thumbWebp,
			smallUrl: smallWebp,
			regularUrl: regularWebp,
			imageUrl: original, // True original file (JPG/PNG)
			base64Thumbnail, // Tiny base64 PNG for instant blur-up (like Unsplash)
			// AVIF variants will be null initially, generated in background
			imageAvifUrl: null,
			thumbnailAvifUrl: null,
			smallAvifUrl: null,
			regularAvifUrl: null,
			// Flag to indicate AVIF generation is pending
			avifPending: true,
		};
	} catch (err) {
 ...
 * Called after critical formats are saved to DB
 * @param {Buffer} buffer - Original image buffer
 * @param {string} bucket - R2 bucket name
 * @param {string} filename - Base filename (without extension)
 * @returns {Promise<Object>} Object with AVIF URLs
 */
export async function generateAvifFormats(buffer, bucket, filename) {
	try {
		logger.info(`[AVIF] Phase 2: Generating AVIF formats in background for ${filename}...`);
		const avifStart = Date.now();
		
		// Generate AVIF variants in parallel (optimized effort: 2-3 for faster encoding)
		const [
			thumbnailAvifBuffer,
			smallAvifBuffer,
			regularAvifBuffer,
			...
			// AVIF sizes (optimized effort: 2 for faster encoding, still good quality)
			sharp(buffer).resize(200, 200, { fit: 'cover' }).avif({ quality: 50, effort: 2 }).toBuffer(),
			sharp(buffer).resize(500, 500, { fit: 'cover' }).avif({ quality: 50, effort: 2 }).toBuffer(),
			sharp(buffer).resize(1000, 1000, { fit: 'inside' }).avif({ quality: 50, effort: 2 }).toBuffer(),
			sharp(buffer).avif({ quality: 55, effort: 3 }).toBuffer(), // Full-size AVIF (slightly higher effort for quality)
		]);
		
		// Upload AVIF formats to R2
		const [
			thumbAvif,
			smallAvif,
			regularAvif,
			...
			uploadToR2(avifFullBuffer, `${bucket}/${filename}.avif`, 'image/avif'),
		]);
		
		const avifMs = Date.now() - avifStart;
		logger.info(`[AVIF] Phase 2 complete in ${avifMs}ms - AVIF formats uploaded for ${filename}`);
		
		return {
			imageAvifUrl: avifFull,
			thumbnailAvifUrl: thumbAvif,
			smallAvifUrl: smallAvif,
			regularAvifUrl: regularAvif,
		};
	} catch (err) {
		logger.error(`[AVIF] Failed to generate AVIF formats for ${filename}:`, err?.message);
		// Don't throw - AVIF is optional, WebP will be used as fallback
		return {
			imageAvifUrl: null,
			thumbnailAvifUrl: null,
			smallAvifUrl: null,
...
export const deleteImageFromR2 = async (publicId, folder = 'photo-app-images') => {
	try {
		// Use prefix-based deletion so we don't have to keep this in sync
		// with all the different formats/sizes we generate (webp, avif, original, etc.)
		const prefix = `${folder}/${publicId}`;
		const keysToDelete = [];
		...
		// Delete all matching objects in parallel
		await Promise.all(
