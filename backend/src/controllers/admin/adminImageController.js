...
import { sendImageApprovedEmail, sendImageRejectedEmail } from '../../utils/emailAlerts.js';
...
export const moderateImage = asyncHandler(async (req, res) => {
    ...
    // Populate user for email notification
    await image.populate('uploadedBy', 'email displayName username');

    // Send email notification based on status (async, don't block)
    if (status === 'approved') {
        Promise.resolve(sendImageApprovedEmail(image.uploadedBy, image)).catch(err => {
            console.error('Failed to send approval email:', err);
        });
    } else if (status === 'rejected') {
        const rejectionReason = image.moderationNotes || 'Your image did not meet our quality standards or content guidelines.';
        Promise.resolve(sendImageRejectedEmail(image.uploadedBy, image, rejectionReason)).catch(err => {
            console.error('Failed to send rejection email:', err);
        });
    }
    // Note: 'flagged' status doesn't send email (may need review)
