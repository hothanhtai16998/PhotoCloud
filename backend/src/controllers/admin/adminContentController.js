...
export const rejectContent = asyncHandler(async (req, res) => {
    ...
    // Send email notification (async, don't block)
    Promise.resolve(sendImageRejectedEmail(image.uploadedBy, image, reasonStr || 'Your image did not meet our quality standards or content guidelines.')).catch(err => {
    ...
    await SystemLog.create({
        level: 'info',
        message: `Content rejected: ${contentId}${reasonStr ? ` - Reason: ${reasonStr}` : ''}`,
        userId: req.user._id,
        action: 'rejectContent',
        metadata: { contentId, reason: reasonStr },
    });
